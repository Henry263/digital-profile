...
// services/qrService.js - MongoDB ONLY storage (no file system)
const QRCode = require('qrcode');
const sharp = require('sharp');
const fs = require('fs').promises;
const path = require('path');
...
class QRService {
  constructor() {
    // Keep directories for backward compatibility (uploaded QR codes only)
    this.uploadDir = path.join(__dirname, '../uploads');
    this.qrDir = path.join(this.uploadDir, 'qr-codes');
    // DO NOT create qr-codes directory anymore
  }

  // ============================================
  // MONGODB-ONLY QR CODE GENERATION
  // ============================================

  /**
   * Generate QR code for MongoDB storage ONLY
   * Returns base64 data URL - NO file system storage
   */
  async generateQRForDB(profile, options = {}) {
    try {
      const {
      ...
      const standaloneUrl = `${baseURL}/card/${profile.slug || profile.cardId}`;
      
      console.log(`🔲 Generating ${type} QR code for MongoDB: ${profile.name}`);
      ...
      // Generate QR code as data URL (base64) - MONGODB ONLY
      const qrDataURL = await QRCode.toDataURL(standaloneUrl, qrOptions);

      console.log(`✅ ${type} QR code generated (${qrDataURL.length} bytes) - stored in MongoDB only`);
      ...
      return {
        ...
        format: 'PNG',
        generatedAt: new Date()
        // NO fileInfo - completely removed file storage
      };
  ...
  /**
   * Generate multiple QR formats for MongoDB storage ONLY
   */
  async generateMultipleFormats(profile) {
    try {
      console.log(`🔲 Generating multiple QR formats for MongoDB: ${profile.name}`);
      ...
      console.log(`✅ Generated ${formats.length} QR formats - ALL stored in MongoDB only`);
      return formats;
  ...
  /**
   * Regenerate QR codes for MongoDB storage ONLY
   */
  async regenerateQRCodes(profile) {
    try {
      console.log(`🔄 Regenerating QR codes for MongoDB: ${profile.name}`);
      
      // Generate new QR codes for MongoDB ONLY
      const newQRCodes = await this.generateMultipleFormats(profile);
      
      console.log(`✅ Regenerated ${newQRCodes.length} QR code formats - stored in MongoDB only`);
  ...
  // ============================================
  // CUSTOM QR GENERATION (MongoDB only)
  // ============================================

  /**
   * Generate custom QR code with logo - MongoDB storage only
   */
  async generateCustomQR(data, options = {}) {
    const {
      size = 400,
  ...
  /**
   * Delete old QR code files (for cleanup only)
   * This is now only for cleaning up old files from previous versions
   */
  async deleteOldQRCodes(qrCodes) {
    console.log('🧹 Cleaning up old QR code files (if any)...');
    try {
      for (const qr of qrCodes) {
        ...
        if (qr.fileInfo && qr.fileInfo.filename) {
          ...
          console.log(`🗑️ Deleted old file: ${qr.fileInfo.filename}`);
        }
        // Also handle legacy format
        if (qr.filename) {
          const filePath = path.join(this.qrDir, qr.filename);
          await fs.unlink(filePath).catch(() => {});
          console.log(`🗑️ Deleted legacy file: ${qr.filename}`);
        }
      }
      console.log('✅ Old QR files cleanup complete');
    } catch (error) {
      console.error('Error deleting old QR codes:', error);
    }
  }
  ...
  /**
   * @deprecated Use generateQRForDB instead
   * This method is kept only for backward compatibility
   */
  async generateProfileQR(profile, options = {}) {
    console.warn('⚠️ generateProfileQR is deprecated. Use generateQRForDB instead.');
    
    // Redirect to MongoDB-only generation
    const qrData = await this.generateQRForDB(profile, {
      ...options,
  ...
  /**
   * @deprecated File storage is no longer used
   */
  async saveQRToFile(dataURL, profile, type) {
    console.warn('⚠️ saveQRToFile is deprecated. QR codes are now stored in MongoDB only.');
    return null; // Do nothing - MongoDB storage only
  }
}
